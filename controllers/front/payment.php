...
class VismaPayPaymentModuleFrontController extends ModuleFrontController
{
	public function postProcess()
	{
		...
		if (!$auth) {
			die($this->module->l('Payment method is not available.', 'payment'));
		}
		...
		if(($discounts) != 0)
		{
			array_push($products, array(
				'id' => 1,
				'title' => $this->module->l('Total discounts', 'payment'),
				'count' => 1,
		...
		else
		{
			...
			else
			{
				...
				catch (Visma\VismaPayException $e) 
				{
					...
					$error = $this->module->l('Payment failed, please try again.', 'payment');
					...
					$this->context->smarty->assign('vp_error', $error); 
			...
			{
				$error = $this->module->l('Visma Pay: No payment methods available for the currency: ', 'payment') . $order_currency;
				$vp_url = $cart_url = $this->context->link->getPageLink('cart',
					null,
					$this->context->language->id,
					['action' => 'show']
		...
		try
		{
			...
			{
				$error = $this->module->l('Payment failed, please try again. (Error code ', 'payment'). $response->result .')';

				if($response->result == 10)
					$error = $this->module->l('Visma Pay system is currently in maintenance. Please try again in a few minutes.', 'payment');
